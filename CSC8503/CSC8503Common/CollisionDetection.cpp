	...
	const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) 
{
	...
	if (deltaLength < radii)
	{
		...
		Vector3 localA = normal * volumeA.GetRadius(); // Get the exact point of collision on the sphere edge
	...
	const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
    ...
    //spherePos = invTransform * spherePos;

    Vector3 delta = spherePos - obbPos;
    Vector3 invDelta = invTransform * delta;

    Vector3 closestPointOnBox = Maths::Clamp(invDelta, -obbHalfSize, obbHalfSize);
    ...
    if (distance < sphereRadius)
    {
        ...
        //collisionNormal = (transform * spherePos - closestPointOnBox).Normalised();
...
bool CollisionDetection::CapsuleSphereIntersection(
	...
	const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) 
{
	...
	float capsuleHalfSize = volumeA.GetHalfHeight() - capsuleRadius;
	Vector3 capsuleNormal = worldTransformA.GetOrientation().ToEuler().Normalised();

	// Get positions at the very end and beginning of capsule shape
	Vector3 tempPos = Vector3(sin(capsuleNormal.x) * capsuleHalfSize, cos(capsuleNormal.y) * capsuleHalfSize, sin(capsuleNormal.z) * capsuleHalfSize);
	Vector3 start = tempPos + worldTransformA.GetPosition();
	Vector3 end = -tempPos  + worldTransformA.GetPosition();	

	Vector3 edgesDir = (end - start).Normalised();

	Vector3 delta = (spherePos - start);

	float sphereProj = Vector3::Dot(delta, edgesDir);
	...
	Vector3 closestPoint = start + (edgesDir * sphereProj);

	// Distance from closest point to capsule edge
	float distance = (closestPoint - start).Length();

	if ((capsuleHalfSize*2) <= distance)
			localPoint = end; // bottom edge
	else if (sphereProj < 0.0f)
			localPoint = start; // top edge
	else
			localPoint = closestPoint; // at closest point

	float distanceToLocalPoint = (localPoint - spherePos).Length();
	
	float radii = capsuleRadius + sphereRadius;
	
	if (distanceToLocalPoint > radii) 
		return false;	// Too far

	// Calculate new distance
	distance = (spherePos - localPoint).Length();
	...
	const OBBVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	Vector3 obbHalfSize = volumeB.GetHalfDimensions();
	...
	Matrix3 transform = obbOrientation;
	Matrix3 invTransform = Matrix3(obbOrientation.Conjugate());

	///delta = localPoint - obbPos; // local point doesn't work for some reason
	delta = capsulePos - obbPos;
	Vector3 invDelta = invTransform * delta;

	Vector3 closestPointOnBox = Maths::Clamp(invDelta, -obbHalfSize, obbHalfSize);
	localPoint = invDelta - closestPointOnBox;
...
bool CollisionDetection::CapsuleAABBIntersection(const CapsuleVolume& volumeA, const Transform& worldTransformA,
	const AABBVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	Vector3 obbPos = worldTransformB.GetPosition();
	Vector3 obbHalfSize = volumeB.GetHalfDimensions();
	...
	Vector3 delta = obbPos - capsulePos;

	Vector3 closestPointOnOBB = Maths::Clamp(delta, -obbHalfSize, obbHalfSize);

	Vector3 localPoint = delta - closestPointOnOBB;
	float distance = localPoint.Length();
